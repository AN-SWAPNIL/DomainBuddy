...
CREATE INDEX IF NOT EXISTS idx_subdomains_subdomain_name ON subdomains(subdomain_name);
CREATE INDEX IF NOT EXISTS idx_subdomains_record_type ON subdomains(record_type);
CREATE INDEX IF NOT EXISTS idx_subdomains_status ON subdomains(status);
CREATE INDEX IF NOT EXISTS idx_subdomains_is_active ON subdomains(is_active);
CREATE INDEX IF NOT EXISTS idx_subdomains_created_at ON subdomains(created_at);

-- Enable Row Level Security (RLS) - Optional since using custom auth
-- ALTER TABLE subdomains ENABLE ROW LEVEL SECURITY;

-- Note: RLS policies are commented out because this project uses custom JWT auth
-- instead of Supabase Auth. Access control is handled in the application layer.

-- If you want to enable RLS with custom auth, you would need to set up
-- a custom auth context function or handle it differently.

-- Create policies for subdomains table (COMMENTED OUT - using custom auth)
/*
CREATE POLICY "Users can view own domain subdomains" ON subdomains
    FOR SELECT USING (
        domain_id IN (
            SELECT id FROM domains WHERE owner_id = auth.uid()
        )
    );

CREATE POLICY "Users can insert subdomains for own domains" ON subdomains
...
CREATE POLICY "Users can update own domain subdomains" ON subdomains
    FOR UPDATE USING (
        domain_id IN (
            SELECT id FROM domains WHERE owner_id = auth.uid()
        )
...
CREATE POLICY "Users can delete own domain subdomains" ON subdomains
    FOR DELETE USING (
        domain_id IN (
            SELECT id FROM domains WHERE owner_id = auth.uid()
        )
